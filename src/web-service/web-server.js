...
export async function startWebServer(api) {
  ...
  io.on("connection", (socket) => {
    console.log(chalk.green(`ID: ${socket.id} -> đã kết nối tới web socket`));
    ...
    socket.on("updateSelectedGroups", (selectedGroups) => {
      const configPath = botInfo.WEB_CONFIG_PATH;
    ...
    socket.on("disconnect", () => {
      console.log(chalk.red(`Client ${socket.id} đã ngắt kết nối khỏi web socket`));
  ...
  const PORT = botInfo?.webPort || 3300;
