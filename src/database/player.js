...
export async function logout(idUserZalo) {
  try {
    ...
    if (result.affectedRows === 0) {
      return { success: false, message: `Bạn chưa đăng nhập bất kỳ tài khoản nào!` };
    }
...
export async function banPlayer(idUserZalo) {
  try {
    ...
    return { success: true, message: `Người chơi đã bị ban thành công!` };
  } catch (error) {
    console.error("Lỗi khi ban người chơi:", error);
    return { success: false, message: `Đã xảy ra lỗi khi ban người chơi!` };
  }
}
...
export async function unbanPlayer(idUserZalo) {
  try {
    ...
    return { success: true, message: `Đã gỡ ban người chơi thành công!` };
  } catch (error) {
    console.error("Lỗi khi unban người chơi:", error);
    return { success: false, message: `Đã xảy ra lỗi khi gỡ ban người chơi!` };
  }
}
...
export async function getMyCard(api, idUser) {
  try {
    ...
    if (rows.length === 0) {
      return {
        success: false,
        message: `Chưa có thông tin, vui lòng kiểm tra lại thông tin đăng nhập.\nDùng lệnh game để xem hướng dẫn. ❌`,
      };
    }
    ...
    return { success: false, message: `Đã xảy ra lỗi khi lấy thông tin. ❌` };
  }
}
...
export async function setLoserGame(idUser, amount) {
  try {
    ...
    if (result.affectedRows === 1) {
      return { success: true, message: `Cập nhật lượt thua thành công. ✅` };
    } else {
      return { success: false, message: `Cập nhật lượt thua thất bại. ❌` };
    }
  } catch (error) {
    console.error("Lỗi khi cập nhật lượt thua:", error);
    return { success: false, message: `Đã xảy ra lỗi khi cập nhật lượt thua. ❌` };
  }
}
...
export async function updatePlayerBalance(idUser, amount, isWin = null, numAmountWin) {
  try {
    ...
    return { success: false, message: `Đã xảy ra lỗi khi cập nhật số dư. ❌` };
  }
}
...
export async function updateAccountVND(username, amount) {
  try {
    ...
    if (result.affectedRows === 1) {
      return { success: true, message: `Cập nhật số dư VND thành công. ✅` };
    } else {
      return { success: false, message: `Cập nhật thất bại. ❌` };
    }
  } catch (error) {
    console.error("Lỗi khi cập nhật số dư VND của tài khoản:", error);
    return { success: false, message: `Đã xảy ra lỗi khi cập nhật số dư VND!` };
  }
}
...
export async function registerAccount(username, password) {
  try {
    ...
    if (specialChars.test(password) || vietnameseChars.test(password) || specialChars.test(username) || vietnameseChars.test(username)) {
      return {
        success: false,
        message: `Tài khoản hoặc mật khẩu Không được chứa ký tự đặc biệt hoặc dấu. Vui lòng thử lại (lưu ý bỏ dấu [ và ])!`,
      };
    }
    ...
    return {
      success: true,
      message: `Đăng ký tài khoản thành công, dùng lệnh daily để nhận quà hàng ngày!`,
    };
  } catch (error) {
