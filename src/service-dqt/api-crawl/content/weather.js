...
async function getOverallWeather(api, message, threadId) {
  try {
    ...
    await api.sendMessage(
      { msg: "Đã xảy ra lỗi khi lấy thông tin thời tiết tổng quan. Vui lòng thử lại sau.", quote: message,ttl: 6000 },
      threadId,
      message.type
    );
  }
...
async function getLocalWeather(api, message, threadId, location, isOverall = false) {
  try {
    ...
    if (!geoData.results || geoData.results.length === 0) {
      await api.sendMessage(
        { msg: "Không tìm thấy thành phố. Vui lòng kiểm tra lại tên thành phố.", quote: message ,ttl: 6000},
        threadId,
        message.type
      );
      return;
    ...
    await api.sendMessage({ msg: weatherInfo, quote: message  ,ttl:60000 }, threadId, message.type);
  } catch (error) {
    console.error("Lỗi khi lấy thông tin thời tiết địa phương:", error);
    await api.sendMessage(
      { msg: "Đã xảy ra lỗi khi lấy thông tin thời tiết địa phương. Vui lòng thử lại sau.", quote: message ,ttl:6000 },
      threadId,
      message.type
    );
  }
