...
export async function handleBauCua(api, message, groupSettings) {
  ...
  if (requestBalance.success) {
    ...
    await api.sendMessage({ msg: `${requestBalance.message}`, quote: message }, threadId, message.type);
    return;
  }

  if (Object.keys(bets).length === 0) {
    await api.sendMessage(
      { msg: `Vui lòng đặt cược theo định dạng: !baucua [loại] [số tiền]:[loại] [số tiền]...`, quote: message },
      threadId,
      message.type
    );
    return;
  ...
  if (requestData.success) {
    ...
    if (bigNumBalance.lt(totalBet)) {
      await api.sendMessage(
        { msg: `${status} Số dư Không đủ. Bạn chỉ có ${formatCurrency(bigNumBalance)} VNĐ.`, quote: message },
        threadId,
        message.type
      );
      return;
    ...
    await api.sendMessage({ msg: `${status} ${requestData.message}`, quote: message }, threadId, message.type);
    return;
  }
