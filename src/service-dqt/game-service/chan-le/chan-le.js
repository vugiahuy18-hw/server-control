...
export async function handleChanLe(api, message, groupSettings) {
  ...
  if (parts.length !== 2) {
    await api.sendMessage(
      { msg: `Vui lòng đặt cược theo định dạng: ${prefix}chanle [chẵn/lẻ] [số tiền/all/10%/100k/1m/1b]`, quote: message },
      threadId,
      message.type
    );
    return;
  ...
  try {
    ...
    if (betAmount.lt(1000)) {
      await api.sendMessage({ msg: `Số tiền cược tối thiểu là 1,000 VNĐ`, quote: message }, threadId, message.type);
      return;
    }
  } catch (error) {
  ...
  if (new Big(balance.balance).lt(betAmount)) {
    await api.sendMessage(
      { msg: `Số dư Không đủ. Bạn chỉ có ${formatCurrency(new Big(balance.balance))} VNĐ`, quote: message },
      threadId,
      message.type
    );
    return;
