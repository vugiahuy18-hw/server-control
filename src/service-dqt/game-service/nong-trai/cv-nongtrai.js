  ...
  if (!cropImages[cacheKey]) {
    try {
      ...
      cropImages[cacheKey] = await loadImage(
        path.join(process.cwd(), "src", "service-dqt", "game-service", "nong-trai", "res", "caytrong", imagePath)
      );
    } catch (error) {
...
async function getSoilImage() {
  if (!soilImage) {
    soilImage = await loadImage(path.join(process.cwd(), "src", "service-dqt", "game-service", "nong-trai", "res", "dat", "dat_tot.png"));
  }
  return soilImage;
}
  ...
  try {
    ...
    const images = await Promise.all(
      buildingImages.map(img => 
        loadImage(path.join(process.cwd(), 'src', 'service-dqt', 'game-service', 'nong-trai', 'res', 'zone', img))
      )
    );
