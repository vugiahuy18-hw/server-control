...
// Đường dẫn file
const BLACKLIST_PATH = path.join('C:', 'Users', 'Administrator', 'Downloads', 'ndqfull', 'src', 'service-dqt', 'spamsms', 'blacklist.json');
const ADMIN_PATH = path.join('C:', 'Users', 'Administrator', 'Downloads', 'ndqfull', 'assets', 'data', 'list_admin.json');
...
export async function handleSpamSmsCommand(api, message) {
  ...
  await sendMessageStateQuote(api, message, `${getCleanNameServer()}Đang tiến hành spam\nSDT: ${sdt}\nSố Lần: ${count}\nCreate By: HwH`, true, 60000, false);

  // ✅ Đổi sang file spamsms.py
  const pythonScriptPath = path.join('C:', 'Users', 'Administrator', 'Downloads', 'ndqfull', 'src', 'service-dqt', 'spamsms', 'spamsms.py');
