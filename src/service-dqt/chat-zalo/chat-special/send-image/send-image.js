...
const CONFIG = {
  saveDir: tempDir,
  baseDataPath: path.resolve(process.cwd(), "src", "service-dqt", "chat-zalo", "chat-special", "send-image", "data"),
  maxAttempts: 30,
  minImageSize: 10,
  ...
  secureRetry: {
    ...
    domains: ['imgur.com', 'i.imgur.com', 'staticflickr.com', 'live.staticflickr.com'] // Các domain cần xử lý đặc biệt
  }
};
...
const IMAGE_TYPES = {
  girl: {
    variants: {
      ...
      nude: { api: "https://api.sumiproject.net/images/nude", ttl: 300000, type: "Khỏa Thân" },
      cosplay: { source: "cosplay.txt", ttl: 300000, type: "Cosplay" },
      anime: { api: "https://api.sumiproject.net/images/anime", ttl: 300000, type: "Anime" },
    },
...
export async function sendHwDev(api, message, caption, ttl = 0) {
  ...
  const imageFiles = fs.readdirSync(imageDir).filter(f => f.startsWith("hwdev"));
