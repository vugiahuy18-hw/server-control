...
export async function handleLeaveGroup(api, message) {
 ...
 // if (senderId === idBot) return;
  await sendMessageResultRequest(api, MessageType.GroupMessage, threadId, "Bai mấy cu, bố đi đây!", true, 1000);
...
export async function handleLeaveAllGroup(api, message) {
  ...
  try {
    ...
    for (const g of groups) {
      try {
        await safeSend(api, g.groupId, "ALL BOX");
    ...
    await safeSend(api, threadId, `✅ Đã thoát ${success} nhóm trong tổng ${groups.length}.`);
  } catch (e) {
...
export async function handleLeaveLockedGroups(api, message) {
  ...
  await sendMessageResultRequest(api, MessageType.GroupMessage, threadId, "🔍 Đang quét các box...", true, 180000);
  ...
  try {
    ...
    for (const group of groups) {
      try {
        ...
        // In ra để debug cấu trúc thật
        console.log(`👉 Check group ${group.groupName} (${group.groupId}):`, JSON.stringify(groupInfo, null, 2));
        ...
        if (isLocked) {
          ...
          console.log(`🚪 Left group ${group.groupName} (${group.groupId})`);
          leftCount++;
        }
      } catch (err) {
