...
// Danh sách admin của bot mẹ (dùng để kế thừa quyền cho bot con)
const motherAdminListPath = path.join(projectRoot, "assets", "json-data", "list_admin.json");
...
export async function createAllRequiredFiles(uidFrom, args, botConfig) {
  ...
  const ownerUid = uidFrom.toString();

  // Lấy danh sách admin của bot mẹ (nếu có) để kế thừa sang bot con
  let motherAdmins = [];
  try {
    if (fs.existsSync(motherAdminListPath)) {
      const raw = fs.readFileSync(motherAdminListPath, "utf8");
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) {
        motherAdmins = parsed.map((id) => id.toString());
      }
    }
  } catch (error) {
    console.error(`Không thể đọc danh sách admin bot mẹ: ${error.message}`);
  }

  const childAdminList = Array.from(new Set([ownerUid, ...motherAdmins]));

  const fileMap = {
    [path.join(projectRoot, botConfig.configFilePath)]: credentialsData,
    [path.join(projectRoot, botConfig.groupSettingsPath)]: {},
    // Đảm bảo: chủ bot con + toàn bộ admin bot mẹ đều là admin của bot con
    [path.join(projectRoot, botConfig.adminFilePath)]: childAdminList,
