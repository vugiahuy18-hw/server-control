...
➤ ${prefix}${aliasCommand} getlogin: Đăng nhập bot thông qua mã QR`;
  ...
  try {
    ...
    // Main bot luôn có quyền admin
    if (api.is_main_bot) {
      isMotherBotAdmin = true;
    } else {
      try {
      ...
      case "update":
        if (args.length < 3) {
          return await sendMessageWarning(
            api,
            message,
            `Sai cú pháp!\n\nCách dùng:\n• ${prefix}${aliasCommand} ${subCommand} credentials [Imei] [Cookie]\n• ${prefix}${aliasCommand} ${subCommand} name [tên mới]\n• ${prefix}${aliasCommand} ${subCommand} description [mô tả mới]`,
            true,
            false
          );
        }
        ...
        if (updateType === "credentials") {
          if (args.length < 5) {
            return await sendMessageWarning(
              api,
              message,
              `Sai cú pháp!\nCách dùng: ${prefix}${aliasCommand} ${subCommand} credentials [Imei] [Cookie]`,
              true,
              false
            );
          }
          await updateBotCredentials(api, message, args.slice(3), aliasCommand);
          ...
          if (!newName) {
            return await sendMessageWarning(
              api,
              message,
              `Sai cú pháp!\nCách dùng: ${prefix}${aliasCommand} ${subCommand} name [tên mới]`,
              true,
              false
            );
          }
          await handleUpdateName(api, uidFrom, dName, newName, threadId, type, message);
          ...
          if (!newDescription) {
            return await sendMessageWarning(
              api,
              message,
              `Sai cú pháp!\nCách dùng: ${prefix}${aliasCommand} ${subCommand} description [mô tả mới]`,
              true,
              false
            );
          }
          await handleUpdateDescription(api, uidFrom, dName, newDescription, threadId, type, message);
        } else {
          await sendMessageWarning(api, message, "Chỉ hỗ trợ update: credentials, name hoặc description", true, false);
        }
        break;
      case "getupdate":
        ...
        break;
      case "help":
        const helpMessage = `Hướng dẫn căn bản:
1️⃣ Các lệnh cơ bản:
...
2️⃣ Các lệnh quản lý bot của quản trị viên: 
➤ ${prefix}${aliasCommand} manager: Xem danh sách lệnh quản lý bot`;
      ...
      case "manager":
        ...
        const qtvMessage = `Bảng hướng dẫn quản lý bot:
...
export async function deleteBot(api, message, isAdminLevelHighest, args) {
  ...
  try {
    ...
    if (removeSuccess) {
      ...
      await sendMessageWarning(api, message, "Đã xảy ra lỗi gì đó, vui lòng thử lại sau", true, false);
      return false;
    }
  } catch (error) {
...
async function extendBotExpiry(api, message, isAdminLevelHighest, timeStr, index) {
  ...
  try {
    ...
    if (timeStr === "-1") {
      ...
      // Xử lý trường hợp expiryAt là "-1" hoặc date string
      let currentExpiry;
      if (botInfo.expiryAt === "-1") {
        currentExpiry = new Date();
      } else {
        currentExpiry = new Date(botInfo.expiryAt);
    ...
    // Active bot nếu hết hạn mới là vĩnh viễn hoặc còn hạn và bot đang ở trạng thái expired/trialExpired/stopped
    if (newExpiry === "-1" || new Date(newExpiry) > currentTime) {
      const shouldActivate = ["expired", "trialExpired", "stopped"].includes(botInfo.status);
      const pm2Status = await checkPM2Status(uidFrom);
      
      if (shouldActivate && !pm2Status.running) {
        // Thử khởi động bot nếu chưa chạy
        if (await fs.access(launcherPath).then(() => true).catch(() => false)) {
          const startSuccess = await startBotWithLauncher(uidFrom);
          if (startSuccess) {
            await updateBotStatus(uidFrom, "running");
      ...
      } else if (pm2Status.running) {
        // Bot đã chạy, cập nhật status thành running
        await updateBotStatus(uidFrom, "running");
        statusMessage += `\n📊 Trạng thái: Đang hoạt động`;
      } else if (shouldActivate) {
        // Bot cần được active nhưng không thể khởi động
        await updateBotStatus(uidFrom, "stopped");
        statusMessage += `\n📊 Trạng thái: Đã dừng`;
      }
    }
...
async function getBotUidByIndex(index) {
  try {
    ...
    const myBots = JSON.parse(await fs.readFile(myBotsPath, "utf8"));
    // Sắp xếp danh sách bot theo createdAt để đảm bảo thứ tự ổn định
    const botList = Object.keys(myBots).sort((a, b) => {
      const timeA = myBots[a].createdAt ? new Date(myBots[a].createdAt).getTime() : 0;
      const timeB = myBots[b].createdAt ? new Date(myBots[b].createdAt).getTime() : 0;
      return timeA - timeB;
    });
    
    if (index < 1 || index > botList.length) {
      console.warn(`Số thứ tự ${index} không hợp lệ. Tổng số bot: ${botList.length}`);
      return null;
    }
    const botUid = botList[index - 1];
    console.log(`Lấy bot số thứ tự ${index}: ${botUid} (tổng: ${botList.length})`);
    return botUid;
  } catch (error) {
...
async function handleListBots(api, threadId, type, message) {
  try {
    ...
    const myBots = JSON.parse(await fs.readFile(myBotsPath, "utf8"));
    // Sắp xếp danh sách bot theo createdAt để đảm bảo thứ tự ổn định (giống getBotUidByIndex)
    const sortedBotKeys = Object.keys(myBots).sort((a, b) => {
      const timeA = myBots[a].createdAt ? new Date(myBots[a].createdAt).getTime() : 0;
      const timeB = myBots[b].createdAt ? new Date(myBots[b].createdAt).getTime() : 0;
      return timeA - timeB;
    });
    const botList = sortedBotKeys.map(key => myBots[key]);
    if (botList.length === 0) {
      return await sendMessageWarning(api, message, "Danh sách bot trống!", true, false);
    }
